  ...
  module Admin
    class OrdersController < Spree::Admin::BaseController
      ...
      def index
        ...
        # Reporte diario
        @daily_orders = Spree::Order.complete.where(updated_at: (Time.now.beginning_of_day..Time.now.end_of_day))
        #@daily_orders = Spree::Shipment.first.manifest.first
        respond_to do |format|
          format.html
          format.csv {
            filename = "Ventas-#{Time.now.strftime("%Y%m%d")}.csv"
            send_data(@daily_orders.to_csv, :type => "text/csv; charset=utf-8; header=present", :filename => filename)
          }
        end
      end
      ...
      def export_csv
        # Format CSV
        @orders_two = Spree::Order.all
        format.csv {
          filename = "Ventas-#{Time.now.strftime("%Y%m%d")}.csv"
          send_data(@orders_two.to_csv, :type => "text/csv; charset=utf-8; header=present", :filename => filename)
        }
      end
